...
- Current
  ...
  - Why: Produce the first Root persistent-prop state row for Props resolver/runtime integration.
  - Summary: Root Props — Anchor To Pilot.
  - Status: ready/local

---

# Root Props — Anchor To Pilot
...
```text
Use IMAGE 1 only as the established Playthings steampunk visual style reference.

Create one clean transparent horizontal 8-state sprite row for one persistent generic Root-world prop: a compact brass-and-dark-wood utility crate/tool chest with readable mechanical details.

Keep the exact same prop identity across all 8 states.

Column 1 default.
Column 2 active/operating.
Column 3 alternate/open.
Column 4 carried/attached-ready compact state.
Column 5 damaged/degraded.
Column 6 repaired/recovered.
Column 7 materialized/placed.
Column 8 removed/consumed/broken.

Leave clear empty space between every state.
Do not let neighboring states touch or overlap.
No text, labels, UI, or scenery.
```
...
- place-style-seed
  ...
  - Purpose: steampunk visual style authority
